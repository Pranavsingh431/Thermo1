...
Usage:
    python simulate_live_feed.py --source_dir /path/to/images \
        --interval_sec 10 --substation_code SUB001
...
class ThermalImageProcessor:
    ...
    async def process_image(self, image_path: Path,
        ...
        try:
            ...
            data.add_field('ambient_temperature', '34.0')
            data.add_field('notes', f'Simulated FLIR T560 capture - '
            ...
            async with self.session.post(upload_url, data=data) as response:
                ...
                if response.status == 201:
                    # Success
                    response_data = await response.json() if \
                        response.content_type == 'application/json' else {}
                    logger.info(f"✅ Successfully processed "
                                f"{image_path.name}")
                    analysis_id = response_data.get('analysis_id', 'Unknown')
            ...
            logger.error(f"💥 Unexpected error processing "
                         f"{image_path.name}: {e}")
            self.failed_count += 1
            return False

    async def _move_to_processed(self, image_path: Path,
                                 processed_dir: Path):
        ...
        try:
            ...
            logger.error(f"Failed to move {image_path.name} to "
                         f"processed archive: {e}")
        ...
        return {
            ...
            "success_rate": (self.processed_count /
                             max(1, self.processed_count +
                                 self.failed_count)) * 100,
            "runtime_seconds": runtime,
            "processing_rate": self.processed_count / max(1, runtime / 60)
        }
...
class LiveFeedSimulator:
    ...
    def __init__(self, source_dir: str, interval_sec: int,
                 substation_code: str,
            ...
            async with self.processor.session.get(health_url) as response:
                if response.status == 200:
                    ...
                    logger.info(f"   🗄️  Database: "
                                f"{health_data.get('database', 'unknown')}")
                else:
                    logger.warning(f"⚠️ API health check returned status "
                                   f"{response.status}")
    ...
    async def run(self):
        ...
        try:
            while self.running:
                ...
                if new_images:
                    logger.info(f"🔍 Found {len(new_images)} new "
                                f"thermal images")
                    ...
                    for image_path in new_images:
                        ...
                        await self.processor.process_image(
                            image_path, self.processed_dir)

                        # Add to processed set
                        self.processed_files.add(image_path.name)

                # Print statistics every 5th cycle
                if (self.processor.processed_count > 0 and
                        self.processor.processed_count % 5 == 0):
    ...
    def _print_statistics(self):
        ...
        logger.info(f"   🚀 Processing rate: "
                    f"{stats['processing_rate']:.1f} images/minute")
...
async def main():
    ...
    if not os.path.exists(args.source_dir):
        logger.error(f"Source directory does not exist: "
                     f"{args.source_dir}")
        sys.exit(1)

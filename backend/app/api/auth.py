...
from app.utils.auth import (
    ...
    create_refresh_token_pair, rotate_refresh_token, verify_refresh_token
)
...
async def login_for_access_token(
    ...
    device_info = (
        f"{request.headers.get('user-agent', 'unknown')}|{client_ip}"
...
async def refresh_token(
    ...
    try:
        ...
        new_refresh_token, _ = rotate_refresh_token(
            db, data.token, device_info
        )
        if not new_refresh_token:
            raise HTTPException(
                status_code=401, detail="Token rotation failed"
            )
